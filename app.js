...
const elements = {
  ...
  eventImagePlaceholder: document.getElementById("event-image-placeholder"),
  eventTitle: document.getElementById("event-title"),
  eventDate: document.getElementById("event-date"),
  eventHost: document.getElementById("event-host"),
  eventLocation: document.getElementById("event-location"),
  eventTown: document.getElementById("event-town"),
  eventStartDisplay: document.getElementById("event-start-display"),
  eventEndDisplay: document.getElementById("event-end-display"),
  ...
  hostImagePlaceholder: document.getElementById("host-image-placeholder"),
...
function availableTimezones(event) {
  const raw = String(event?.timezones || "")
    .split(",")
    .map((item) => item.trim().toUpperCase())
    .filter(Boolean);
  const valid = raw.filter((item) => TIME_ZONE_MAP[item]);
  return valid.length ? valid : ["EST"];
}
...
function setZone(zone) {
  ...
  const startDate = parseEventDateTime(state.featuredEvent, state.featuredEvent.startTime);
  const endDate = parseEventDateTime(state.featuredEvent, state.featuredEvent.endTime);
  const startLabel = formatClock(startDate, zone);
  const endLabel = endDate ? formatClock(endDate, zone) : "End time TBD";

  elements.eventStartDisplay.textContent = startLabel;
  elements.eventEndDisplay.textContent = endLabel;
...
function renderHome() {
  ...
  if (!featured) {
    ...
    elements.eventTown.textContent = "-";
    elements.eventStartDisplay.textContent = "-";
    elements.eventEndDisplay.textContent = "-";
    elements.eventTimeRange.textContent = "-";
    elements.hostCardName.textContent = "-";
  ...
  elements.eventDescription.textContent = featured.subtitle || featured.description || "Add a short event summary.";
  elements.eventLongDescription.textContent = featured.description || "Add a fuller event description from host sign in.";
  ...
  if (featured.eventImageUrl) {
    elements.eventImage.src = featured.eventImageUrl;
    elements.eventImage.hidden = false;
    elements.eventImagePlaceholder.hidden = true;
  } else {
    elements.eventImage.hidden = true;
    elements.eventImage.removeAttribute("src");
    elements.eventImagePlaceholder.hidden = false;
  }

  if (featured.hostImageUrl) {
    elements.hostImage.src = featured.hostImageUrl;
    elements.hostImage.hidden = false;
    elements.hostImagePlaceholder.hidden = true;
  } else {
    elements.hostImage.hidden = true;
    elements.hostImage.removeAttribute("src");
    elements.hostImagePlaceholder.hidden = false;
  }
  ...
  elements.timezoneNote.textContent =
    zones.length > 1
      ? `Switch between the time zones selected by the host for ${formatDateLabel(featured.startDate)}.`
      : `The host selected a single timezone for ${formatDateLabel(featured.startDate)}.`;
