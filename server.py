...
SEEDED_EVENT = {
    ...
    "subtitle": "A beautiful afternoon of celebration, fine dining, and laughter dedicated to the mothers who inspire us every day.",
    ...
    "hostImageUrl": "",
    "description": (
        "Join us for a special Mother's Day Lunch as we celebrate and honor the incredible women in our lives. "
        "Expect warm company, beautiful details, and a relaxed afternoon together."
    ),
    "aboutTitle": "About the Lunch",
    "rsvpDeadline": "RSVP by May 5th to confirm your place.",
    "menuTitle": "Menu Preview",
    "menuItems": "Smoked Salmon Carpaccio | With capers, dill, and lemon zest | Starter\nHerb-Crusted Spring Lamb | With roasted heirloom carrots and mint reduction | Main",
    "specialTitle": "Special Instructions",
    "specialInstructions": "Complimentary valet parking available at the main entrance.\nPlease let us know of any severe allergies when you RSVP.\nDress code: Garden Party Elegant.",
    "featured": True,
    "emoji": "MD",
    "capacity": "",
    "timezones": "EST,IST",
...
def initials_from_name(value: str) -> str:
    parts = [part for part in str(value).strip().split() if part][:2]
    if not parts:
        return "I"
    return "".join(part[0].upper() for part in parts)


def normalize_event_record(row: dict[str, object]) -> dict[str, object]:
    return {
        "id": str(row["id"]),
        "title": str(row["title"]),
        ...
        "aboutTitle": str(row["aboutTitle"]),
        "rsvpDeadline": str(row["rsvpDeadline"]),
        "menuTitle": str(row["menuTitle"]),
        "menuItems": str(row["menuItems"]),
        "specialTitle": str(row["specialTitle"]),
        "specialInstructions": str(row["specialInstructions"]),
...
def enrich_events(events: list[dict[str, object]], rsvps: list[dict[str, object]]) -> list[dict[str, object]]:
    stats = {str(event.get("id", "")): {"goingGuestCount": 0, "guestPreview": []} for event in events}

    for rsvp in rsvps:
        event_id = str(rsvp.get("eventId", "")).strip()
        if not event_id or str(rsvp.get("response", "")).strip() != "Going":
            continue
        bucket = stats.setdefault(event_id, {"goingGuestCount": 0, "guestPreview": []})
        bucket["goingGuestCount"] += max(safe_int(rsvp.get("guestCount", 1), 1), 1)
        initials = initials_from_name(str(rsvp.get("name", "")))
        if initials and initials not in bucket["guestPreview"] and len(bucket["guestPreview"]) < 3:
            bucket["guestPreview"].append(initials)

    enriched = []
    for event in events:
        event_id = str(event.get("id", ""))
        payload = dict(event)
        payload.update(stats.get(event_id, {"goingGuestCount": 0, "guestPreview": []}))
...
def upsert_seed_event(sheet) -> None:
    ...
    featured_index = EVENT_HEADERS.index("featured")
    for row in sheet.iter_rows(min_row=2, values_only=False):
        row[featured_index].value = False
...
def workbook_get_events() -> list[dict[str, object]]:
    ...
    events = [normalize_event_record(row) for row in rows]
...
def workbook_upsert_event(payload: dict[str, object]) -> None:
    ...
    with WORKBOOK_LOCK:
        ...
        row_values = [
            ...
            str(payload.get("aboutTitle", "")).strip(),
            str(payload.get("rsvpDeadline", "")).strip(),
            str(payload.get("menuTitle", "")).strip(),
            str(payload.get("menuItems", "")).strip(),
            str(payload.get("specialTitle", "")).strip(),
            str(payload.get("specialInstructions", "")).strip(),
        ...
        featured_column = header_map.get("featured", EVENT_HEADERS.index("featured") + 1)
        if row_values[19]:
            for row in sheet.iter_rows(min_row=2, values_only=False):
...
def google_upsert_event(payload: dict[str, object]) -> None:
    ...
    updated_event = {
        ...
        "aboutTitle": str(payload.get("aboutTitle", "")).strip(),
        "rsvpDeadline": str(payload.get("rsvpDeadline", "")).strip(),
        "menuTitle": str(payload.get("menuTitle", "")).strip(),
        "menuItems": str(payload.get("menuItems", "")).strip(),
        "specialTitle": str(payload.get("specialTitle", "")).strip(),
        "specialInstructions": str(payload.get("specialInstructions", "")).strip(),
...
def get_events() -> list[dict[str, object]]:
    if google_connected():
        try:
            return enrich_events(google_get_events(), google_get_rsvps())
        except Exception:
            return enrich_events(workbook_get_events(), workbook_get_rsvps())
    return enrich_events(workbook_get_events(), workbook_get_rsvps())
...
def build_rsvp_export_bytes() -> bytes:
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "RSVPs"
    sheet.append(["Created At", "Event", "Name", "Email", "Phone", "Guests", "Note", "Response"])

    for row in get_rsvps():
        sheet.append(
            [
    ...
    for column in sheet.columns:
        ...
        sheet.column_dimensions[column[0].column_letter].width = min(max(width, 12), 40)
...
class InvitelyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/uploads/"):
            self.serve_upload_file(parsed.path)
            return
        ...
        if parsed.path == "/api/admin/rsvps/export":
            self.require_admin_session()
            self.send_xlsx(build_rsvp_export_bytes(), "invitely-rsvps.xlsx")
            return
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/admin/upload":
            try:
                self.require_admin_session()
                payload = self.handle_admin_upload()
                self.send_json(payload)
        ...
        try:
            ...
            if parsed.path == "/api/admin/session":
                ...
                self.send_json({"ok": True}, cookies=[self.build_session_cookie(session_token)])
                return
    ...
    def serve_upload_file(self, request_path: str) -> None:
        filename = unquote(request_path.removeprefix("/uploads/"))
        if not filename or "/" in filename or "\\" in filename or ".." in Path(filename).parts:
            self.send_error(HTTPStatus.NOT_FOUND, "Not found")
            return
        self.serve_file(UPLOAD_DIR / filename)

    def read_form_data(self) -> tuple[dict[str, str], dict[str, object] | None]:
        ...
        if length > MAX_UPLOAD_BYTES + 512_000:
        ...
        for part in message.iter_parts():
            ...
            if filename:
                ...
                fields[field_name] = payload.decode(part.get_content_charset() or "utf-8", errors="ignore").strip()
        return fields, upload

    def handle_admin_upload(self) -> dict[str, object]:
        fields, upload = self.read_form_data()
        if not upload:
        ...
        content_type = str(upload.get("content_type", "")).split(";", 1)[0].strip().lower()
        if content_type not in ALLOWED_UPLOAD_TYPES:
            raise ValueError("Upload a JPG, PNG, WEBP, or GIF image.")

        original_suffix = Path(str(upload.get("filename", ""))).suffix.lower()
        allowed_suffixes = {".jpg", ".jpeg", ".png", ".webp", ".gif"}
        suffix = original_suffix if original_suffix in allowed_suffixes else ALLOWED_UPLOAD_TYPES[content_type]
        kind = slugify(fields.get("kind", "image"))
        filename = f"{kind}-{uuid.uuid4().hex[:12]}{suffix}"
        target = UPLOAD_DIR / filename
        target.write_bytes(data)
        return {"ok": True, "url": f"/uploads/{quote(filename)}"}
    ...
    def send_xlsx(self, body: bytes, filename: str) -> None:
        self.send_response(HTTPStatus.OK)
        self.send_header(
            "Content-Type",
